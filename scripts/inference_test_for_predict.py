...
class Predictor:
    def __init__(self, model_name="RF2_apr23",interactive=False, model_param={}, loader_param={}, crop_size=256, maxcycle=4):
        ...
        self.model_subname = "1.0.0"#"samechain"#"on_rigid" #"on_RF2"
        ...
        with torch.no_grad():
            ...
            for inputs in test_hl_ag_loader:
                ...
                if True:
                    ...
                    write_pdb(msa[:,0,0][0], xyz_prev[0], L_s=L_s[0], Bfacts=epitope_info[0].bool(),prefix=f"templates_check/{item[0]}_templates_{i_epi_res}th_epi") #ProjectName
                    ...
                    for i_cycle in range(N_cycle):
                        with ExitStack() as stack:
                            ...
                            if i_cycle < N_cycle - 1:
                                ...
                                if i_cycle % 4 == 3:
                                    write_pdb(
                                        ...
                                        prefix=f"{i_epi_res}th_epi_{item[0]}_{i_cycle}",       
                                    )
                                continue
                            else:
                            ...
                            # print('one_pae check\n', one_pae)
                            # check the nan in probab
                            has_nan = np.isnan(probab).any()
                            # print('has_nan',has_nan)
                            
                            # print('probab[0][:20].shape',probab[0][:20].shape)
                            distogram_contact_prob_12 = probab[0][:12].sum(axis=0)
                            # print('distogram_contact_prob_12.shape',distogram_contact_prob_12.shape)
                            # print('distogram_contact_prob_12[0,1]',distogram_contact_prob_12[0,1])
                            # plt.imshow(distogram_contact_prob_12, cmap="jet", interpolation="nearest", vmin=0, vmax=1.1)
                            # plt.colorbar()
                            # plt.title(f"distogram_contact_prob_12[pred]")
                            # plt.subplot(2, 2, 2)
                            # print('probab_pae.shape',probab_pae.shape)
                            # print('np.array(list_pae)',np.array(list_pae).shape)
                            # pae = torch.sum(probab_pae[0][:12], dim=0).detach().cpu().numpy()
                            ...
                            print(f'{item[0]} interface_pae_mean: {interface_pae_mean:.2f}')
                            pae_dict[str(item[0])][f"{i_epi_res}th_epi"] = interface_pae_mean

                os.makedirs(f"inference_pdb/{self.model_name}_{self.model_subname}_After210930/{item[0]}", exist_ok=True)
                with open(f"inference_pdb/{self.model_name}_{self.model_subname}_After210930/{item[0]}/pae_dict.csv", 'w') as f:
                    writer = csv.writer(f)
    ...
    def inference(self, rank, world_size):
        ...
        for cluster in hl_ag_test_2.keys():
            for item in hl_ag_test[cluster]:
                print(f"item {item}")
                if os.path.exists(f"/home/kkh517/submit_files/Project/epitope_sampler_halfblood/inference_pdb/{self.model_subname}_After210930/{item}/pae_dict.csv"):

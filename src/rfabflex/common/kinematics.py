...
def xyz_to_t2d(xyz_t, mask, params=PARAMS):
    ...
    Returns
    -------
    t2d : pytorch tensor of shape [batch,templ, nres,nres,37+6+1]
          stores stacked dist,omega,theta,phi 2D maps
    """
    B, T, L = xyz_t.shape[:3]
...
def c6d_to_bins2(c6d, same_chain, negative=False, params=PARAMS):
    """bin 2d distance and orientation maps
    Input: c6d - (B, L, L, 4)
    0: dist, 1: omega, 2: theta, 3: phi
    """

    dstep = (params["DMAX"] - params["DMIN"]) / params["DBINS"]  # (20-2)/36 = 0.5
    astep = 2.0 * np.pi / params["ABINS"]  # 2*pi/36
...
def get_init_xyz_compl(xyz_t, same_chain):
    ...
    B, T, L = xyz_t.shape[:3]
    init = (
        INIT_CRDS.to(xyz_t.device).reshape(1, 1, 1, 27, 3).repeat(B, T, L, 1, 1)
        + torch.rand(B, T, L, 1, 3, device=xyz_t.device) * 5.0
    )
    ...
    for i_b in range(B):
        for i_T in range(T):
            ...
            is_same_chain_in_templ = same_chain[i_b][
                :, ~mask[i_b, i_T]
            ].bool()  # (L, L_sub)
            seqmap = (

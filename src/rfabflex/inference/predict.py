...
sys.path.append('/home/kkh517/submit_files/Project/OnlyRigid_pair/src')
from rfabflex.data.parsers import parse_a3m, read_templates, read_template_pdb, parse_pdb, read_template_pdbs
from rfabflex.model.RoseTTAFoldModel  import RoseTTAFoldModule
from rfabflex.common.util import * # check util
from collections import namedtuple
from rfabflex.data.ffindex import *
from featurizing import MSAFeaturize, MSABlockDeletion
from rfabflex.common.kinematics import xyz_to_c6d, xyz_to_t2d
from rfabflex.common.chemical import INIT_CRDS
from rfabflex.model.util_module import XYZConverter
from rfabflex.common.symmetry import symm_subunit_matrix, find_symm_subs, get_symm_map
sys.path.append('/home/kkh517/submit_files/Project/OnlyRigid_pair/scripts')
from training.train_RF2_rigid import EMA
from training.data_loader_rigid2 import merge_a3m_hetero
from training.argument import get_args
import json
import random
...
#     parser.add_argument("-monomer_dir", default=None, type=str, help="Directory containing monomer templates.  If not provided, assert error:(")
...
class Predictor():
    def __init__(self, model_name, device="cuda:0"):
        # define model name
        model_weights = f"/home/yubeen/rf_abag_templ/results/0919_test/models/weights/{model_name}.pt"
        self.model_weights = model_weights
        self.device = device
        self.active_fn = nn.Softmax(dim=1)

        # define model & load model
        self.model = EMA(RoseTTAFoldModule(
            **MODEL_PARAM
        ).to(self.device), 0.99)
        ...
        self.gpu = rank
        could_load = self.load_model(self.model_weights, rank)
        if not could_load:
            print ("ERROR: failed to load model")
            sys.exit()

        # get model
        self.ddp_model = DDP(self.model, device_ids=[self.gpu], find_unused_parameters=False)
    ...
    def load_model(self, model_weights, rank): #
        if not os.path.exists(model_weights):
            return False
        # checkpoint = torch.load(model_weights, map_location=self.device)
        map_location = {"cuda:0": f"cuda:{rank}"}
        checkpoint = torch.load(model_weights, map_location=map_location)
    ...
    def predict(
        ...
        n_templ=4, msa_mask=0.0, is_training=False, monomer_dir=None,msa_concat_mode="diag"
    ):
        
        # self.xyz_converter = self.xyz_converter.cpu() # why?
        ...
        for i,seq_i in enumerate(inputs):
            ...
            if (len(fseq_i) == 3):
                ...
                xyz_t_i, t1d_i, mask_t_i = read_template_pdbs(Ls_blocked[i],monomer_dir,fseq_i[0])
    ...
    def run_prediction(
        ...
        writepdb("%s_pred.pdb"%(out_prefix), best_xyzfull[0], seq_full[0], L_s, bfacts=100*best_lddtfull[0])
...
if __name__ == "__main__":
    ...
    if (torch.cuda.is_available()):
        ...
        pred = Predictor(args.model_name, torch.device("cuda:0"))
    else:
        print ("Running on CPU")
        pred = Predictor(args.model_name, torch.device("cpu"))

...
class Dropout(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.broadcast_dim is not None:
            shape[self.broadcast_dim] = 1  # reduce target dim to 1
...
def get_topk(D, sep, top_k=64, kmin=32):
    ...
    D_neigh, E_idx = torch.topk(D, min(top_k, L - 1), largest=False)
...
def make_graph_w_2nodes(xyz, pair, idx, top_k_BB=64, top_k_SC=64, kmin=32, eps=1e-6):
    ...
    G.edata["rel_pos"] = (xyz.view(-1, 3)[tgt] - xyz.view(-1, 3)[src]).detach()
...
class XYZConverter(nn.Module):
    ...
    def compute_all_atom(self, seq, xyz, alphas, non_ideal=True, use_H=True):
        ...
        RTF0 = torch.eye(4).repeat(B, L, 1, 1).to(device=Rs.device)  # [B, L, 4, 4]

        # bb
        RTF0[:, :, :3, :3] = Rs
        RTF0[:, :, :3, 3] = Ts

        # omega #[B, L, 4, 4], [B, L, 4, 4], [B, L, 4, 4] -> [B, L, 4, 4]
        RTF1 = torch.einsum(
            "brij,brjk,brkl->bril",
            RTF0,
            self.RTs_in_base_frame[seq, 0, :],
    ...
    def get_torsions(self, xyz_in, seq, mask_in=None):
        B, L = xyz_in.shape[:2]  # [B, L, 27, 3]

        tors_mask = self.get_tor_mask(seq, mask_in)  # [B, L, 10]
        ...
        xyz[..., 0, :] = torch.einsum("brij,j->bri", Rs, Nideal) + Ts  # [B, L, 3]
        xyz[..., 2, :] = torch.einsum("brij,j->bri", Rs, Cideal) + Ts  # [B, L, 3]

        torsions = torch.zeros((B, L, 10, 2), device=xyz.device)
        # cos, sin for last dimension
        ...
        # torsion_indices = [22, 4, 4]
        # [B, L, 4, 3] = ([B, L, 27, 3], 2, [B, L, 4, 3]) #first atom coordinate
        ti1 = torch.gather(
            xyz, 2, self.torsion_indices[seq, :, 1, None].repeat(1, 1, 1, 3)
        )
        # second atom coordinate
        ti2 = torch.gather(
        ...
        torsions[:, :, 3:7, :] = th_dih(ti0, ti1, ti2, ti3)  # [B, L, 4]
        #
        ...
        # torsions - [B, L, 10, 2]
        tors_mask *= ~torch.isnan(torsions[..., 0])  # [B, L, 10]

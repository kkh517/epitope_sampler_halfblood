...
from rfabflex.common.util import cross_product_matrix
from rfabflex.model.util_module import *
from rfabflex.model.util_module import make_topk_graph
from rfabflex.model.Attention_module import *
from rfabflex.model.SE3_network import SE3TransformerWrapper
    ...
    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, top_k=64, eps=1e-5):
        ...
        if top_k != 0:
            G, edge_feats = make_topk_graph(
                xyz[:, :, 1, :].detach(), edge, idx, top_k=top_k
            )
        else:
...
class IterBlock(nn.Module):
    ...
    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, use_checkpoint=False):
        rbf_feat = rbf(torch.cdist(xyz[:, :, 1, :], xyz[:, :, 1, :])) + self.pos(
            idx
        )  # add positional encoding to the distance feature
        if use_checkpoint:
            ...
            pair = checkpoint.checkpoint(
                create_custom_forward(self.pair2pair), pair, rbf_feat, state
            )
...
class IterativeSimulator(nn.Module):
    ...
    def forward(
        ...
        T_s = list()
        alpha_s = list()
        for i_m in range(self.n_extra_block):  # 4
        ...
        for i_m in range(self.n_main_block):  # 12
            R_in = R_in.detach()
            T_in = T_in.detach()  # detach rotation (for stability)
            # Get current BB structure
            xyz = einsum("bnij,bnaj->bnai", R_in, xyz_in) + T_in.unsqueeze(-2)
        ...
        state = self.proj_state2(state) # change state dimension to match the topk SE3 layer
        for i_m in range(self.n_ref_block):  # 4
            R_in = R_in.detach()
            T_in = T_in.detach()  # detach rotation (for stability)
            xyz = einsum("bnij,bnaj->bnai", R_in, xyz_in) + T_in.unsqueeze(-2)
